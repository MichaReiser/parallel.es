...
export class DefaultWorkerThread implements IWorkerThread {
     ...
     */
    constructor(private functionLookupTable: DynamicFunctionRegistry, private communicationChannel: IWorkerThreadSlaveCommunicationChannel) {
        const that = this;
    ...
    public initialize() {
        if (this.state.name !== "default") {
            throw new Error(`The worker thread can only be initialized if in state default but actual state is '${this.state.name}'.`);
        }
    ...
    public run(task: ITaskDefinition, callback: (error: any, result: any) => void): void {
        if (this.state.name !== "idle") {
            throw new Error(`The worker thread can only execute a new task if in state idle but actual state is '${this.state.name}'.`);
        }
        ...
        this.state = new WorkerThreadExecutingState(onComplete, this.functionLookupTable, this.communicationChannel);
    }
