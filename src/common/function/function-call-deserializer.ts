...
export class FunctionCallDeserializer {
	 ...
	 */
	public deserializeFunctionCall<TResult>(
		...
		if (!func) {
			throw new Error(
				`The function with the id ${functionCall.functionId.identifier} could not be retrieved while deserializing the function call. Is the function correctly registered?`,
			);
		}
