...
export class DefaultThreadPool implements IThreadPool {
	...
	public set maxThreads(limit: number) {
		if (typeof limit !== "number" || limit % 1 !== 0 || limit <= 0) {
			throw new Error(
				`The maxThreads limit (${limit}) has to be a positive integer larger than zero.`,
			);
		}
	...
	constructor(
		private workerThreadFactory: IWorkerThreadFactory,
		options: { maxConcurrencyLevel: number },
	) {
	...
	private schedulePendingTasks(): void {
		while (this.queue.length) {
			...
			if (
				...
				this.workers.length < this.maxThreadsLimit
			) {
