 ...
 */
export type INextCallback<TSubResult> = (
 ...
 * @param TEndResult type of the end result
 */
export class ParallelStream<TSubResult, TEndResult>
	implements IParallelStream<TSubResult, TEndResult>
{
	...
	public static transform<TIn, TIntermediate, TResult>(
		...
		let next:
			| ((subResult: TIn, taskIndex: number, valuesPerTask: number) => void)
			| undefined;
		let resolve: ((result: TResult) => void) | undefined;
		let reject: ((reason: any) => void) | undefined;

		const transformedStream = new ParallelStream<TIn, TResult>(
			(nxt, rsolve, rject) => {
				next = nxt;
				resolve = rsolve;
				reject = rject;
		...
		inputStream.subscribe(next!, reject!, (result) =>
