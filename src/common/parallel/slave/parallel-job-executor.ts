...
} from "../../function/serialized-function-call";
import {
	ISerializedParallelOperation,
...
function createTaskEnvironment(
	definition: IParallelJobDefinition,
	functionCallDeserializer: FunctionCallDeserializer,
): IParallelTaskEnvironment {
	let taskEnvironment: IParallelTaskEnvironment = {
		taskIndex: definition.taskIndex,
		valuesPerTask: definition.valuesPerTask,
	};
 ...
 */
export function parallelJobExecutor<T, TResult>(
	definition: IParallelJobDefinition,
	...
	const generatorFunction = functionCallDeserializer.deserializeFunctionCall(
	...
	for (const operation of definition.operations) {
		const iteratorFunction = functionCallDeserializer.deserializeFunctionCall<
			Iterator<T>
		>(operation.iterator);
		const iteratee = functionCallDeserializer.deserializeFunctionCall(
			operation.iteratee,
		);
