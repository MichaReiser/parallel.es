...
export class ParallelTimesGenerator<T> implements IParallelGenerator {
    ...
    public serializeSlice(index: number, numberOfItems: number, functionCallSerializer: FunctionCallSerializer): ISerializedFunctionCall {
        ...
        if (typeof this.iteratee === "function") {
            ...
            iterateeFunction = functionCallSerializer.serializeFunctionCall(ParallelWorkerFunctionIds.IDENTITY, this.iteratee);
        }

        return functionCallSerializer.serializeFunctionCall(ParallelWorkerFunctionIds.TIMES, sliceStart, sliceEnd, iterateeFunction);
    }
}
