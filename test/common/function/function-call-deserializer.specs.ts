...
import {
	functionId,
	IFunctionId,
} from "../../../src/common/function/function-id";
describe("FunctionCallDeserializer", function () {
	...
	beforeEach(function () {
		...
		deserializer = new FunctionCallDeserializer({
	...
	describe("deserializeFunctionCall", function () {
		...
		it("throws if the function is not in the registry", function () {
			...
			// act, assert
			expect(() =>
				deserializer.deserializeFunctionCall(serializedCall),
			).toThrowError(
		...
		it("deserializes the serialized function calls in the params, if deserializeParams is true", function () {
			...
			getFunctionSpy.and.callFake(function (
				funcId: IFunctionId,
			): Function | undefined {
