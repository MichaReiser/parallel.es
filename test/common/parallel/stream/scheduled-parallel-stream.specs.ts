...
describe("ScheduledParallelStream", function () {
	...
	describe("subscribe", function () {
		it("calls the onNext handler for every resolved sub result", function (done) {
			// arrange
			const stream: IParallelStream<string, string> =
				new ScheduledParallelStream(tasks, undefined, joiner);
		...
		it("calls the onRejected handler if any tasks failed", function (done) {
			...
			stream.then(
				function () {
						...
						"The computation of task 2 failed, therefore the promise should have been rejected",
					);
				},
				function (reason) {
