...
describe("AbstractParallelScheduler", function () {
	...
	describe("schedule", function () {
		...
		it("calls the generator.serializeSlice for each task to spawn", function () {
			...
			const serializeSliceSpy = spyOn(
				generator,
				"serializeSlice",
			).and.returnValue({ functionId: 2 });
		...
		it("schedules a task for each slice according to the job", function () {
			...
			serializeFunctionCallSpy.and.callFake(
				(call: FunctionCall): ISerializedFunctionCall => {
			...
			spyOn(generator, "serializeSlice").and.returnValues(
				generatorSlice1,
				generatorSlice2,
			);
