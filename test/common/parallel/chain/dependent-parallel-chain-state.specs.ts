...
describe("DependentParallelChainState", function () {
	...
	beforeEach(function () {
		scheduler = jasmine.createSpyObj<IParallelJobScheduler>("scheduler", [
			"schedule",
		]);
		...
		previousStream = jasmine.createSpyObj<IParallelStream<string[], string[]>>(
			"previousStream",
			["then"],
		);
	...
	describe("resolve", function () {
		it("returns a new scheduled parallel chain state", function () {
			// arrange
			const state = new DependentParallelChainState(
				previousStream,
				options,
				environment,
			);
		...
		it("schedules the tasks as soon as the previous stream has completed", function () {
			...
			// act
			previousStreamThenSpy.calls
				.argsFor(0)[0]
				.apply(undefined, [["a", "b", "c"]]); // previous stream completed
	...
	describe("chainOperation", function () {
		...
		it("returns a that also contains the given operation", function () {
			...
			expect(chained).toEqual(
				new DependentParallelChainState(previousStream, options, environment, [
					operation,
				]),
			);
		});
	...
	describe("addEnvironment", function () {
		...
		it("changes the environment", function () {
			...
			// assert
			expect(chained).toEqual(
				new DependentParallelChainState(
					previousStream,
					options,
					newEnvironment,
				),
